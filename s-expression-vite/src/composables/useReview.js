...
export function useReview(api, auth, lessons) {
    ...
    const handleReviewCheck = async () => {
        ...
        try {
            ...
            if (ok) {
                reviewResultMessage.value = "✅ Correct! Great job!"
                ...
                // Enhanced error message with context about lesson redirect
                let errorMsg = res?.error || "Incorrect."

                // Add hint about lesson redirect for wrong answers
                if (res?.details?.failures) {
                ...
                errorMsg += "\n💡 Don't worry! You'll get a chance to review the lesson material."
                reviewResultMessage.value = "❌ " + errorMsg

                // The modal will automatically show lesson redirect option after displaying this error
            }
        } catch (e) {
            ...
            reviewResultVisible.value = true
            reviewResultOk.value = false
            reviewResultMessage.value = "❌ " + msg + "\n💡 Consider reviewing the lesson for this topic."
            ...
            try {
                await api.recordAttempt(
                    ...
                    currentProblemForReview.value?.id,
                    text,
                    { ok: false, error: msg, details: null },
                    "review"
                )
            } catch (recErr) {
    ...
    // Add a new method to get lesson info for a problem
    const getLessonForProblem = (problemId) => {
        if (!currentProblemForReview.value) return null
        return lessons.lessons.value.find(l => l.id === currentProblemForReview.value.lesson_id)
    }
